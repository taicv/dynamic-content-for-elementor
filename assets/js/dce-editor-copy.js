...
jQuery(window).on('load', function () {
    ...
    jQuery(document).on('click', '.elementor-context-menu-list__item-copy, .elementor-context-menu-list__item-copy_all_content', function () {
        ...
        var transferData = elementorCommon.storage.get('transfer'); //elementorFrontend.config.elements.data[dce_model_cid]; //.attributes;
...
function dceAddPasteAction(groups, element) {
    //console.log(groups);
    var transferGroup = _.findWhere(groups, { name: 'transfer' });
    //console.log(transferGroup);
    if (!transferGroup) {
            return groups;
    }
    jQuery.each(groups, function( index, value ) {
        if (value.name == 'transfer' || value.name == 'paste') {
            //console.log(value);
...
function dcePastFromClipboard(pasteAction, pasteBtn) {
    //jQuery(document).on('mousedown','.elementor-context-menu-list__item-paste-clip', function() {  
    //iFrameDOM.find(document).on('contextmenu', '.elementor-element', function() {
    console.log('dce paste start');
    if (dceCanJsPaste()) {
        navigator.clipboard.readText()
                .then(text => {
                    // `text` contains the text read from the clipboard
                    dcePasteAction(text, pasteAction, pasteBtn);
                })
                .catch(err => {
        ...
        //jQuery(pasteBtn).append().trigger('click');
        //jQuery('#elementor-preview-responsive-wrapper').append('<textarea id="dce_copy_paste_textarea"></textarea>');   
...
function dceAddCopyPasteFallback(value = '', action = 'copy', cid, pasteAction, pasteBtn) {
    if (jQuery('#dce_copy_paste').length) {
        ...
        jQuery('#elementor-preview-responsive-wrapper').append('<div id="dce_copy_paste" data-cid="'+cid+'"></div>');
...
function dcePasteAction(text, pasteAction, pasteBtn) {
    if (isJson(text)) {
        var transferData = JSON.parse(text);
        //if (transferData.elements.length) {
            elementorCommon.storage.set('transfer', transferData);
