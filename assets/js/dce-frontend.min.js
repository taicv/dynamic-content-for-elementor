...
/*ajaxmodal.min.js*/
;var urlAttuale;var titoloAttuale;var ajaxPage_init=function($tid,scopeId){if(jQuery('#dce-wrap').length==0){jQuery('body').addClass('dce-ajax-page-open');jQuery('body').wrapInner('<div id="dce-outer-wrap"><div id="dce-wrap"></div></div>')}
...
if(jQuery('.ajax-open').length>0){jQuery('.ajax-open').each(function(i,el){var elementSettings_ajaxOpen=get_DCE_ElementSettings(jQuery(this));ajaxPage_init(elementSettings_ajaxOpen.ajax_page_template,jQuery(this).attr('data-id'))})}
/*dce-bgcanvas.min.js*/
...
setup(){this.renderer=new THREE.WebGLRenderer({alpha:!0});this.renderer.setPixelRatio(window.devicePixelRatio);this.renderer.setSize(this.el.offsetWidth,this.el.offsetHeight);this.el.appendChild(this.renderer.domElement);var filter_ascii=Boolean(elementSettings.postprocessing_ascii);if(filter_ascii){this.effectAscii=new THREE.AsciiEffect(this.renderer,' .:-+*=%@#',{invert:!0});this.effectAscii.setSize(this.container.width(),this.container.height());this.effectAscii.domElement.style.color='white';this.effectAscii.domElement.style.backgroundColor='black';this.el.appendChild(this.effectAscii.domElement)}else{this.el.appendChild(this.renderer.domElement)}
this.scene=new THREE.Scene();this.clock=new THREE.Clock(!0)}
loadTextures(){const loader=new THREE.TextureLoader();loader.crossOrigin='';this.texture=loader.load(this.image,texture=>{this.mat.uniforms.uTextureSize.value.set(texture.image.width,texture.image.height);console.log(this.texture)});this.texture.wrapS=THREE.ClampToEdgeWrapping;this.texture.wrapT=THREE.ClampToEdgeWrapping;this.texture.minFilter=THREE.LinearFilter}
...
render(){var delta=this.clock.getDelta();this.renderer.render(this.scene,this.camera);this.composer.render(this.delta);if(this.effectAscii)this.effectAscii.render(this.scene,this.camera)}
init(){this.setup();this.cameraSetup();this.loadTextures();this.createMesh();this.initPostProcessing();this.addListeners()}
onResize(){this.mat.uniforms.uScreenSize.value.set(this.container.width(),this.container.height());this.mesh.scale.set(this.container.width(),this.container.height(),1);this.camera.left=-this.container.width()/2;this.camera.right=this.container.width()/2;this.camera.top=this.container.height()/2;this.camera.bottom=-this.container.height()/2;this.camera.updateProjectionMatrix();this.composer.render();this.renderer.setSize(this.container.width(),this.container.height());this.composer.setSize(this.container.width(),this.container.height());if(this.effectAscii)this.effectAscii.setSize(this.container.width(),this.container.height());if(this.effectSobel){this.effectSobel.uniforms.resolution.value.x=this.container.width()*window.devicePixelRatio;this.effectSobel.uniforms.resolution.value.y=this.container.height()*window.devicePixelRatio}}}
...
changeAndSplittingText(count,old_count)};$(window).on('elementor/frontend/init',function(){elementorFrontend.hooks.addAction('frontend/element_ready/dyncontel-animateText.default',WidgetElements_AnimateTextHandler)})})(jQuery)
