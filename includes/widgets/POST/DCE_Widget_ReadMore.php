...
class DCE_Widget_ReadMore extends DCE_Widget_Prototype {
    ...
    protected function render() {
        ...
        $data_text_effect = '';
        if( $settings['style_effect'] != '' && $settings['type_of_button'] == 'text' && $link) $data_text_effect = ' data-text="'.$content_button_raw.'"';
       
        if ($link) {
            $html .= sprintf('<a id="dce-readmore-'.$this->get_id().'" class="dce-btn-readmore%4$s%5$s%6$s" href="%1$s" %2$s%8$s%7$s>%3$s</a>', $link, $target, $title, $animation_class, $effect_class, $icon_class, $data_text_effect, $nofollow);
        } else {
